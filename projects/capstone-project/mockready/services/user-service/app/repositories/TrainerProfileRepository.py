...
class TrainerProfileRepository:
    ...
    async def get_by_id(self, trainer_profile_id: int) -> TrainerProfile | None:
        result = await self.db.execute(
            select(TrainerProfile).where(TrainerProfile.trainer_profile_id == trainer_profile_id, TrainerProfile.is_active == True)
        )
        return result.scalar_one_or_none()
    
    async def soft_delete(self, trainer_profile_id: int) -> bool:
        result = await self.db.execute(
            select(TrainerProfile).where(TrainerProfile.trainer_profile_id == trainer_profile_id)
        )
